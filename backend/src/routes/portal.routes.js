...
import { maskPatientForBilling } from "../utils/demoSeeder.js";
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const restrictedCount = patients.filter(
      (patient) => patient.privacyLevel === "restricted"
    ).length;

    const outstandingBalance = patients.reduce(
      (sum, patient) => sum + (patient.billingProfile?.outstandingBalance || 0),
      0
    );
      ...
      admin: [
        {
          ...
          value: new Set(
            auditEvents.map((event) => event.actorId?.toString()).filter(Boolean)
          ).size,
    ...
    const highlightPatients = isInsurer
      ? patients.slice(0, 3).map(maskPatientForBilling)
      : patients.slice(0, 3);

    const recentActivity = isInsurer
      ? claims.slice(0, 5).map((claim) => ({
          _id: claim._id,
          action: `claim.${claim.status || "updated"}`,
          details: `${claim.claimNumber} • ${claim.patientName} • ${claim.payerName}`,
          actorRole: "billing",
